function y = CIofCVaR(D, S0, K, H, mu, r, T, sigma, l1, m, alphas,alphahi,alphalo,p,Budget,c,n0,m0,delta,lmin,lmax)
...
delta_t = T / m;
tau = l1 * delta_t;
V0 = calc_BarrierCall(S0, T, r, sigma, K, H);
V0 = sum(V0, 'all');

samplePath1 = zeros(D*2, n0);
samplePath0(1:D, :) = exp((mu - sigma^2 / 2) * delta_t ...
    + sigma * CorL * sqrt(delta_t) * normrnd(0, 1, D, n0)) * S0;
samplePath0(D+1:D*2, :) = exp((log(S0 * samplePath0(1:D, :)) - sqrt(power(log(samplePath0(1:D, :) / S0), 2) - 2 * sigma^2 * delta_t * log(rand(D, n0))))/2);
for l = 2:l1
    samplePath1(1:D, :) = exp((mu - sigma^2 / 2) * delta_t ...
    + sigma * CorL * sqrt(delta_t) * normrnd(0, 1, D, n0)) .* samplePath0(1:D, :);
    samplePath1(D+1:D*2, :) = exp((log(samplePath0(1:D, :) .* samplePath1(1:D, :)) - sqrt(power(log(samplePath1(1:D, :) ./ samplePath0(1:D, :)), 2) - 2 * sigma^2 * delta_t * log(rand(D, n0))))/2);
    samplePath1(D+1:D*2, :) = min(samplePath0(D+1:D*2, :), samplePath1(1:D, :));
    samplePath0 = samplePath1;
end

SampleXn0 = samplePath0;
clear SamplePath0 SamplePath1;
SampleS_N = NestedEsti(sigma,SampleXn0,m0,D,delta_t,r, m-l1); 
SampleYm0 = max(SampleS_N(1:D, :, :) - K(1),0) .* (SampleS_N(D+1:D*2, :, :) >= H);
SampleYm0 = max(SampleS_N(1:D, :, :) - K(2),0) .* (SampleS_N(D+1:D*2, :, :) >= H) + SampleYm0;
SampleYm0 = max(SampleS_N(1:D, :, :) - K(3),0) .* (SampleS_N(D+1:D*2, :, :) >= H) + SampleYm0;
...
SampleYS_N1 = NestedEsti(sigma,SampleXn0(:, Index0(I)), maxM,D,delta_t,r, m-l1); %n0-by-length(I)
SampleYM = max(SampleYS_N1(1:D, :, :) - K(1),0) .* (SampleYS_N1(D+1:D*2, :, :) >= H);
SampleYM = max(SampleYS_N1(1:D, :, :) - K(2),0) .* (SampleYS_N1(D+1:D*2, :, :) >= H) + SampleYM;
SampleYM = max(SampleYS_N1(1:D, :, :) - K(3),0) .* (SampleYS_N1(D+1:D*2, :, :) >= H) + SampleYM;
