...
CorL = temp';
delta_t = T / m;
V0 = calc_V0(S0, delta_t, m, r, sigma, K);
tau = delta_t * l1;
N01 = normrnd(0, 1, D, n, m);
samplePath(1:D, :, 1) = (mu - sigma^2 / 2) * delta_t * ones(D, n) ...
    + sigma * CorL * sqrt(delta_t) * N01(:, :, 1) + log(S0);
samplePath(D+1:D*2, :, 1) = samplePath(1:D, :, 1);
for l = 2:l1
    samplePath(1:D, :, l) = (mu - sigma^2 / 2) * delta_t * ones(D, n) ...
    + sigma * CorL * sqrt(delta_t) * N01(:, :, l) + samplePath(1:D, :, l - 1);
    samplePath(D+1:D*2, :, l) = samplePath(D+1:D*2, :, l - 1) + samplePath(1:D, :, l);
end
for l = (l1 + 1):m
    samplePath(1:D, :, l) = (r - sigma^2 / 2) * delta_t * ones(D, n) ...
    + sigma * CorL * sqrt(delta_t) * N01(:, :, l) + samplePath(1:D, :, l - 1);
    samplePath(D+1:D*2, :, l) = samplePath(D+1:D*2, :, l - 1) + samplePath(1:D, :, l);
end
for l = 2:m
    samplePath(D+1:D*2, :, l) = samplePath(D+1:D*2, :, l) / l;
end
samplePath(:, :, :) = exp(samplePath(:, :, :));
tmp = samplePath(D+1:2*D, :, m);
...
for l = l1:(m - 1)
    for j = 1:D
        ...
        m1 = samplePath(j + D, :, l)';
        varphi(:, 5) = exp(-m1 / 2.0);
        varphi(:, 6) = varphi(:, 1) .* (1 - m1);
        varphi(:, 7) = varphi(:, 2) .* (1 - 2 * m1 + (m1 .^ 2) / 2);
        varphi(:, 8) = varphi(:, 3) .* (1 - 3 * m1 + 3 * (m1 .^ 2) / 2 - (m1 .^ 3) / 6);
        varphi(:, 9) = 1;
        coBM_j = N01(j, :, l + 1)';
...
for l = l1:(m - 1)
    for j = 1:D
        ...
        mat1 = repmat(reshape(varphi, n, 9, 1), [1, 1, 9]);
        mat2 = repmat(reshape(varphi, n, 1, 9), [1, 9, 1]);
        mat1 = mat1 .* mat2; %(n)*(9)*(9)
        mat1 = permute(mat1, [2 3 1]);
        mat1 = reshape(mat1, 9, 9 * n);
        mat1 = ml(:, :, l, j) * mat1;
        mat1 = reshape(mat1, 9, n);
        mat1 = (mat1') * mr(:, :, l, j);
        v2 = v2 + mat1;
        mat1 = repmat(yy, [1, 9]);
        mat1 = mat1 .* varphi;
