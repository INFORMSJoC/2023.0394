...
for l = l1:(m - 1)
    for j = 1:D
        m1 = samplePath(j, :, l)';
        varphi(:, 1) = m1;
        varphi(:, 2) = varphi(:, 1) .* m1;
        varphi(:, 3) = varphi(:, 2) .* m1;
        varphi(:, 4) = varphi(:, 3) .* m1;
        varphi(:, 5) = 1;
        coBM_j = N01(j, :, l + 1)';
...
varphi = ones(n, 4 * D + 1);
varphi(:, 1 : D) = m0;
varphi(:, D+1 : D*2) = varphi(:, 1 : D) .* m0;
varphi(:, D*2+1 : D*3) = varphi(:, D+1 : D*2) .* m0;
varphi(:, D*3+1 : D*4) = varphi(:, D*2+1 : D*3) .* m0;
varphi(:, D*4+1) = 1;
