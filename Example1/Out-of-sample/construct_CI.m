...
N01_2 = normrnd(0, 1, D, n, m);
samplePath_2 = zeros(D, n, m);
samplePath_2(:, :, 1) = exp((mu - sigma^2 / 2) * delta_t * ones(D, n) ...
    + sigma * CorL * sqrt(delta_t) * N01_2(:, :, 1)) * S0;

for l = 2:l1
    samplePath_2(:, :, l) = exp((mu - sigma^2 / 2) * delta_t * ones(D, n) ...
    + sigma * CorL * sqrt(delta_t) * N01_2(:, :, l)) .* samplePath_2(:, :, l - 1);
end
for l = (l1 + 1):m
    samplePath_2(:, :, l) = exp((r - sigma^2 / 2) * delta_t * ones(D, n) ...
    + sigma * CorL * sqrt(delta_t) * N01_2(:, :, l)) .* samplePath_2(:, :, l - 1);
end
tmp = samplePath_2(:, :, m);
tmp = max(tmp - K(1), 0.0) + max(tmp - K(2), 0.0) + max(tmp - K(3), 0.0);
sampleY_2 = V0 * ones(D, n) - tmp / exp((T - tau) * r);
sampleY_2 = sampleY_2'; % (n)*(D)
clear tmp;

for l = l1:(m - 1)
    m0 = (samplePath_2(:, :, l))';
    m0 = bsxfun(@rdivide, bsxfun(@minus, m0, mean(m0)), std(m0)); %(n)*(D)
    coBM = sqrt(delta_t) * CorL * N01_2(:, :, l + 1);
    samplePath_2(:, :, l) = m0';
    N01_2(:, :, l + 1) = coBM;
end
...
for l = l1:(m - 1)
    for j = 1:D
        % find beta
        m1 = samplePath_2(j, :, l)';
        varphi_2(:, 1) = exp(-m1 / 2.0);
        varphi_2(:, 2) = varphi_2(:, 1) .* (1 - m1);
        varphi_2(:, 3) = varphi_2(:, 2) .* (1 - 2 * m1 + (m1 .^ 2) / 2);
        varphi_2(:, 4) = varphi_2(:, 3) .* (1 - 3 * m1 + 3 * (m1 .^ 2) / 2 - (m1 .^ 3) / 6);
        varphi_2(:, 5) = 1;
        coBM_j = N01_2(j, :, l + 1)';
        beta = varphi_2 \ sampleY_2(:, j);
        y_residual = sampleY_2(:, j) - varphi_2 * beta;
        beta = varphi_2 \ (y_residual .* coBM_j / delta_t);
        %inference
        m1 = samplePath(j, :, l)';
        coBM_j = N01(j, :, l + 1)';
        varphi(:, 1) = exp(-m1 / 2.0);
...
%find beta
sampleY_2 = sum(sampleY_2, 2);
m0 = samplePath_2(:, :, l1)';
clear samplePath_2;
varphi_2 = ones(n, 4 * D + 1);
varphi_2(:, 1 : D) = exp(-m0 / 2.0);
varphi_2(:, D+1 : D*2) = varphi_2(:, 1 : D) .* (1 - m0);
varphi_2(:, D*2+1 : D*3) = varphi_2(:, D+1 : D*2) .* (1 - 2 * m0 + (m0 .^ 2) / 2);
varphi_2(:, D*3+1 : D*4) = varphi_2(:, D*2+1 : D*3) .* (1 - 3 * m0 + 3 * (m0 .^ 2) / 2 - (m0 .^ 3) / 6);
varphi_2(:, D*4+1) = 1;
beta = varphi_2 \ sampleY_2; %(4D+1)*1
m0 = samplePath(:, :, l1)';
clear samplePath;
varphi = ones(n, 4 * D + 1);
